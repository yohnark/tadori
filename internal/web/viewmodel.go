...
// redacted export projection; every other field is derived from it and can be
...
func BuildDiagnosticView(diagnosticReport model.DiagnosticReport) (DiagnosticViewModel, error) {
	projection, err := report.Project(diagnosticReport)
	if err != nil {
		return DiagnosticViewModel{}, fmt.Errorf("project report for UI: %w", err)
	}
	diagnosticReport = projection.Report
	canonical, err := json.Marshal(diagnosticReport)
	if err != nil {
		return DiagnosticViewModel{}, fmt.Errorf("marshal projected report for UI: %w", err)
	}
